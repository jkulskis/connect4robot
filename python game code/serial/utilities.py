...
def qualified_name(type_):
    ...
    if isinstance(type_, _Module):
        if type_name in (
            'builtins', '__builtin__', '__main__', '__init__',  # __name__
        ):
            type_name = None
    else:
...
def calling_function_qualified_name(depth=1):
    ...
    """
    >>> def f(): return calling_function_qualified_name()
    >>> print(f())
    f
    """
...
def properties_values(o):
    # type: (object) -> typing.Iterable[typing.Tuple[str, typing.Any]]
    for a in dir(o):
        if a[0] != '_':
            v = getattr(o, a)
...
def read(data):
    # type: (Union[str, IOBase, addbase]) -> typing.Any
    if (

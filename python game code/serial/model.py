# region Backwards Compatibility Imports
...
try:
    from typing import Union, Dict, Any, AnyStr, IO, Sequence, Mapping, Callable
except ImportError:
    Union = Dict = Any = AnyStr = IO = Sequence = Mapping = Callable = None
...
def marshal(
    data,  # type: Any
    types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property, Callable]]]
    value_types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
    item_types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
):
    # type: (...) -> Any
    """
    Recursively converts instances of `serial.model.Object` into JSON/YAML/XML serializable objects.
    """
...
def unmarshal(
    data,  # type: Any
    types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
    value_types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
    item_types=None,  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
):
    # type: (...) -> Any
    ...
    if isinstance(data, Generator):
        data = tuple(data)
    if types is None:
        if isinstance(data, (dict, collections.OrderedDict)) and not isinstance(data, Dictionary):
            data = Dictionary(data, value_types=value_types)
        elif isinstance(
            data,
            (collections.Set, collections.Sequence)) and (not isinstance(data, (str, bytes, native_str, Array))
        ):
    ...
    for type_ in types:
        if isinstance(
            ...
            if issubclass(type_, Object) and isinstance(data, (dict, collections.OrderedDict)):
                try:
                    data = type_(data)
                    matched = True
                    break
                ...
                data,
                (dict, collections.OrderedDict, Dictionary)
            ) and issubclass(
                type_,
                (dict, collections.OrderedDict, Dictionary)
            ):
                ...
                isinstance(data, (collections.Set, collections.Sequence, Generator)) and
                (not isinstance(data, (str, bytes, native_str))) and
                issubclass(type_, (collections.Set, collections.Sequence)) and
...
def validate(
    data,  # type: Model
    types=None,  # type: Optional[Union[type, serial.properties.Property, Object, Callable]]
    raise_errors=True  # type: bool
):
    # type: (...) -> Sequence[str]
    """
    This function verifies that all properties/items/values in an instance of `serial.model.Model` are of the correct
    data type(s), and that all required attributes are present (if applicable). If `raise_errors` is `True` (this is the
    default)--violations will result in a validation error. If `raise_errors` is `False`--a list of error messages will
    be returned if invalid/missing information is found, or an empty list otherwise.
    """
    error_messages = []
    error_message = None
    if types is not None:
...
def version(data, specification, version_number):
    # type: (Model, str, Union[str, int, Sequence[int]]) -> None
    """
    Recursively alters instances of `serial.model.Model` according to version number metadata associated with that
    object's properties. This allows one data model to represent multiple versions of a specification.

    Arguments:

        - data (serial.model.Model)
    ...
    def version_properties(properties):
        # type: (Sequence[serial.properties.Property]) -> Optional[Sequence[serial.meta.Meta]]
        changed = False
        nps = []
...
class Object(Model):
    ...
    def _validate(self, raise_errors=True):
        ...
        for property_name, property in meta.properties.items():
            value = getattr(object_, property_name)
            if value is None:
                if isinstance(property.required, collections.Callable):

...
instances of `serial.model.Model` sub-classes as the `data` parameter/property for `urllib.request.Request`, and to
...
class Data(object):
    ...
    def __init__(
        self,
        data=None,  # type: Optional[Union[bytes, str, Sequence, Set, dict, serial.model.Model]]
        headers=None  # type: Optional[Dict[str, str]]
    ):
        """
        Parameters:

            - data (bytes|str|collections.Sequence|collections.Set|dict|serial.model.Model): The payload.
    ...
    @data.setter
    def data(self, data):
        # type: (Optional[Union[bytes, str, Sequence, Set, dict, serial.model.Model]]) -> None
        self._bytes = None
        if data is not None:
            serialize_type = None
            if 'Content-type' in self.headers:
                ...
                if re.search(r'/yaml\b', ct) is not None:
                    serialize_type = 'yaml'
            if isinstance(data, (serial.model.Model, dict)) or (
                isinstance(data, (collections.Sequence, collections.Set)) and not
                isinstance(data, (str, bytes))
            ):
                data = serial.model.serialize(data, serialize_type or 'json')
